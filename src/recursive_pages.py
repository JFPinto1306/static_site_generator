...
def generate_pages_recursive(basepath,dir_path_content, template_path, dest_dir_path):
    for path in os.listdir(dir_path_content):
        ...
        if os.path.isdir(dir_path_content_full):
            ...
            generate_pages_recursive(basepath,dir_path_content_full, template_path, os.path.join(dest_dir_path+'/'+path))
        elif os.path.isfile(dir_path_content_full):
            # If the path is a file, generate the page
            os.makedirs(dest_dir_path, exist_ok=True)
            generate_page(basepath,dir_path_content_full, template_path, os.path.join(dest_dir_path+'/'+path.replace('.md', '.html')))
